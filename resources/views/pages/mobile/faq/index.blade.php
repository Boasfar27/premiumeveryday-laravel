...
@section('content')
    <div class="container mx-auto px-4 py-8 pt-4">
        <div class="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-lg p-5 mb-6">
            <h1 class="text-2xl font-bold text-white mb-3 text-center">Frequently Asked Questions</h1>
            <p class="text-indigo-100 mb-4 text-sm text-center">Temukan jawaban untuk pertanyaan umum tentang layanan dan
                produk digital kami.</p>

            <!-- Search Bar -->
            <div class="relative">
                <input type="text" id="faq-search" placeholder="Search for answers..."
                    class="w-full px-4 py-2 rounded-full border-0 focus:ring-2 focus:ring-indigo-300 text-gray-900 text-sm"
                    autocomplete="off">
        ...
        <div class="overflow-x-auto -mx-4 px-4 mb-4">
            <div class="flex space-x-2 pb-2" id="category-tabs">
                <button class="category-tab active px-3 py-1.5 rounded-full text-xs font-medium" data-category="all">
                    All FAQs
                </button>
                @foreach (App\Models\Faq::categories() as $categoryKey => $categoryName)
                    @if (in_array($categoryKey, $categories))
                        <button class="category-tab px-3 py-1.5 rounded-full text-xs font-medium bg-gray-100 text-gray-700"
            ...
            @foreach ($faqs as $faq)
                <div x-data="{ open: false }" class="bg-white rounded-lg faq-card" data-category="{{ $faq->category }}">
                    <button @click="open = !open" class="w-full flex justify-between items-center p-4 focus:outline-none">
                        <span class="text-base font-medium text-gray-900 text-left">{{ $faq->question }}</span>
                    ...
                    <div x-show="open" class="px-4 pb-4" x-transition:enter="transition ease-out duration-300"
                        ...
                        <div class="text-sm text-gray-600 faq-content">{!! $faq->answer !!}</div>
                    </div>
                </div>
            @endforeach
        </div>
        ...
        <div id="no-results" class="hidden mt-6 p-4 text-center bg-gray-50 rounded-lg">
            ...
            <p class="mt-1 text-gray-500 text-sm">Try adjusting your search terms</p>
        </div>

        <div class="mt-8 text-center">
            <p class="text-gray-600 text-sm">Still have questions?</p>
            <a href="{{ route('contact') }}"
                class="inline-flex items-center mt-2 px-3 py-1.5 bg-primary text-white text-sm rounded-lg hover:bg-primary-dark transition-colors">
