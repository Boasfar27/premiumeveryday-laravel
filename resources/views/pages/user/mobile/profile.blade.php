    ...
    <div class="min-h-screen bg-gray-50">
        ...
        <div class="bg-gradient-to-r from-pink-500 to-pink-600 p-4">
            <div class="flex flex-col items-center">
                <div
                    class="h-24 w-24 rounded-full bg-white flex items-center justify-center text-3xl font-bold text-pink-500 mb-3">
                    {{ strtoupper(substr(auth()->user()->name, 0, 2)) }}
                </div>
                <h1 class="text-xl font-bold text-white text-center">{{ auth()->user()->name }}</h1>
                <p class="text-pink-200 text-sm">Bergabung sejak {{ auth()->user()->created_at->format('d M Y') }}</p>
            </div>
        </div>
                ...
                <form action="{{ route('user.profile.update') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', auth()->user()->name) }}"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-pink-500 focus:ring focus:ring-pink-500 focus:ring-opacity-50 text-sm">
                        @error('name')
                    ...
                    <button type="submit"
                        class="w-full bg-pink-600 text-white py-2 px-4 rounded-md text-sm font-medium hover:bg-pink-700 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-opacity-50">
                        Simpan Perubahan
                    </button>
                </form>
            </div>
