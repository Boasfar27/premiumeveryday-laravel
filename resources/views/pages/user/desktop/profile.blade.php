        ...
        <div class="bg-white rounded-lg shadow-lg overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-pink-500 to-pink-600 p-8">
                <div class="flex items-center">
                    <div
                        class="h-24 w-24 rounded-full bg-white flex items-center justify-center text-2xl font-bold text-pink-500 shadow-md">
                        {{ strtoupper(substr(auth()->user()->name, 0, 2)) }}
                    </div>
                    <div class="ml-8">
                        <h1 class="text-2xl font-bold text-white">{{ auth()->user()->name }}</h1>
                        <p class="text-pink-200 mt-1">Bergabung sejak {{ auth()->user()->created_at->format('d M Y') }}
                        </p>
                    </div>
                </div>
            </div>
                    ...
                    <div class="bg-gray-50 p-6 rounded-lg shadow-sm">
                        ...
                        <form action="{{ route('user.profile.update') }}" method="POST" class="space-y-5">
                            ...
                            <div class="flex justify-end pt-2">
                                <button type="submit"
                                    class="bg-pink-600 text-white px-5 py-2 rounded-md hover:bg-pink-700 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-opacity-50 shadow-sm">
                                    Simpan Perubahan
                                </button>
                            </div>
                        </form>
